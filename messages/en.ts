export default {
  meta: {
    title: '{name} - Software Engineer | IT Guy',
    description: 'This is the English translation file for the website.',
    keywords:
      '{name}, Software Engineer, IT Guy, Web Developer, Portfolio, Projects, Blog, Contact',
    author: '{name}',
    language: 'en',
  },
  header: {
    home: 'Home',
    about: 'About',
    projects: 'Projects',
    blog: 'Blog',
    contact: 'Contact',
    recipes: 'Recipes',
    career: 'Career',
    github: {
      title: 'GitHub',
      description: 'View my GitHub profile and repositories',
    },
    linkedin: {
      title: 'LinkedIn',
      description: 'View my LinkedIn profile and connect with me',
    },
    youtube: {
      title: 'YouTube',
      description: 'View my YouTube channel and subscribe for updates',
    },
  },
  about: {
    keys: {
      age: 'age',
      name: 'name',
      location: 'location',
      jobTitle: 'jobTitle',
      learning: 'learning',
      interests: 'interests',
      languages: 'languages',
      description: 'description',
      days: 'days',
      years: 'years',
      months: 'months',
      hours: 'hours',
      minutes: 'minutes',
      seconds: 'seconds',
    },
    title: 'About Me',
    jobTitle: 'IT Guy in apprentice',
    location: 'Schaffhausen, Switzerland',
    learning: ['Cyber Security', 'Java'],
    interests: ['Coding', 'Flying drone', 'Climbing'],
    languages: {
      german: 'Native',
      english: 'Fluent',
    },
    downloadResume: 'Download Resume',
    description:
      'I have been coding for over 4 years, beginning my journey in 2021. Initially, I learned HTML, CSS, and JavaScript to build websites.\n\nMy first project was a simple website built with HTML, CSS, and JavaScript (~mid-2021).\nAs I progressed, I first focused on React.js and after a while switched over to Next.js.\n\nI am currently also making a apprenticeship as an IT Guy at Georg Fischer AG, where I lean a lot about how big businesses operate.',
  },
  hero: {
    linkText: 'Read More',
    title: 'This is ME',
    bio: 'I love coding, learning new technologies and flying drone. I am a software engineer with a passion for building anything that is fun.',
    welcome: "Hey, I'm Colin",
    description:
      'I am a software engineer with a passion for building web applications. I love coding and learning new technologies.',
    items: {
      nordkapp: {
        title: 'Nordkapp Trip 2025',
        linkText: 'Watch the video',
        summary:
          '1.5 years ago my Dad sade to me: "Colin, when your 18 and you have your driving license, we will go to Nordkapp". I sad: "Sure, why not". And this is how the idea was born.',
      },
      thisisme: {
        title: 'This is ME',
        linkText: 'This Portfolio',
        summary:
          'This is my personal website where I showcase my projects and skills. I built it using Next.js, TypeScript, Tailwind CSS and many more.',
      },
    },
  },
  buttons: {
    readMore: 'Read More',
    contact: 'Contact Me',
    downloadResume: 'Download Resume',
    viewProjects: 'View Projects',
    viewCode: 'View Code',
    viewGithub: 'View my github',
    viewLinkedIn: 'View my LinkedIn',
    visitWebsite: 'Visit Website',
    viewAll: 'View All',
    sendMessage: 'Send Message',
    send: 'Send',
    close: 'Close',
    learnMore: 'Learn More',
    backToTop: 'Back to Top',
    myWork: 'My Work',
    myProjects: 'My Projects',
    blog: 'Blog',
    projects: 'Projects',
    home: 'Home',
    more: 'More',
  },
  projects: {
    noEndDate: 'now',
    seeMore: 'Want to see more?',
    viewAll: 'View all projects',
    viewGithub: 'View on GitHub',
    visitWebsite: 'Visit Website',
    title: 'My Projects',
    description:
      'Explore some of my recent or not so recent projects below. For more, visit my GitHub profile.',
    items: {
      thisisme: {
        name: 'This is ME',
        description: 'This is my personal website where I showcase my projects and skills.',
      },
      spotifyStats: {
        name: 'Spotify Stats',
        description: 'A Web app to track you Spotify listening history',
      },
    },
  },
  settings: {
    title: 'Settings',
    language: 'Language',
    theme: 'Theme',
    darkMode: 'Dark Mode',
    lightMode: 'Light Mode',
    systemDefault: 'System Default',
    save: 'Save Settings',
    description: 'Here you can change your settings, like website theme or decorations.',
    mouseShadow: 'Mouse Shadow',
    mouseShadowOn: 'On',
    mouseShadowOff: 'Off',
    acrylicAnimation: 'Background Animation',
    acrylicAnimationOn: 'On',
    acrylicAnimationOff: 'Off',
  },
  technologies: {
    title: 'Technology I use',
    description:
      'Over the years, I have worked with a variety of technologies. Here are some of the technologies I have experience with:',
  },
  contact: {
    title: 'Contact Me',
    description:
      "I'm always eager to explore new opportunities and take on exciting projects. If you have a project in mind, or just want to say hi, feel free to send me a message.",
    otherOptions: 'Or contact me with...',
    form: {
      name: 'Name',
      placeholderName: 'Your Name',
      email: 'Email',
      placeholderEmail: 'richi@srf.ch',
      message: 'Message',
      placeholderMessage: 'Hello there, I would like to ask you about...',
      send: 'Send Message',
      success: 'Your message has been sent successfully!',
      error: 'There was an error sending your message. Please try again later.',
      loading: 'Sending message...',
      emptyName: 'Please enter your name.',
      emptyEmail: 'Please enter your email address.',
      invalidEmail: 'Please enter a valid email address.',
      emptyMessage: 'Please enter your message.',
      somethingEmpty: 'Please fill out all fields.',
      characters: 'characters',
      charactersLeft: 'characters left',
      maxLength: 'Maximum length is 500 characters.',
    },
  },
  wakatime: {
    title: 'Wakatime Stats',
    descriptionYear:
      'Wakatime is a tool that tracks the time I spend coding. Here are my stats for all time.',
    descriptionWeek:
      'Wakatime is a tool that tracks the time I spend coding. Here are my stats for the last 7 days.',
    languages: 'Languages',
    editors: 'Editors',
    os: 'Operating Systems',
    overall: 'Overall Activity',
    topEditors: 'Top Editors in hours',
    topLanguages: 'Top Languages in hours',
    totalLanguages: 'Total Languages',
    totalTime: 'Total time',
    topOperatingSystems: 'Top Operating Systems in hours',
    codingActivity: 'Coding Activity',
    last7Days: 'Last 7 Days',
    allTime: 'All Time',
    lastWeek: 'Last Week',
    noStats: 'No Wakatime data available.',
    loading: 'Loading Wakatime stats...',
    dailyAverage: 'Daily Average',
    totalCodingTime: 'Total coding statistics since {startDate}',
    totalCodingTimeWeek: 'Total coding statistics of the last 7 days',
    noLanguageDate: 'No language data available.',
    noEditorData: 'No editor data available.',
    noOperatingSystemData: 'No operating system data available.',
    time: 'Time',
    hours: 'Hours',
  },
  blog: {
    title: 'Tech Blog',
    description:
      'A blog about technology, programming, and various intriguing topics. Here I share my experiences, projects and opinions.',
    keywords:
      'blog, technology, programming, software development, web development, coding, tutorials, guides',
    readMore: 'Read More',
    noPosts: 'No blog posts available.',
    loading: 'Loading blog posts...',
    seeAllPosts: 'See all posts',
    seeAllCategories: 'See all categories',
    allPosts: 'All Posts',
    latestPost: 'Latest Post',
    filterByTech: 'Filter by Technology',
    all: 'All Technologies',
    words: 'words',
    published: 'Published',
    by: 'by',
    on: 'on',
    read: 'read',
    tableOfContents: 'Table of Contents',
  },
  career: {
    title: 'My Career',
    description:
      'My career journey so far. Here are some of the companies I have worked for and the roles I have held.',
    experienceTitle: 'Experience',
    educationTitle: 'Education',
    now: 'Now',
    present: 'Present',
    items: {
      wibilea: {
        company: 'Wibilea',
        department: 'Base Year',
        role: 'IT Guy in apprentice',
        description:
          'At the beginning of my apprentice I attended the Wibilea Basis Year in Neuhausen. I learned a lot about the basics of IT and business.',
      },
      gfLocalIt: {
        company: 'Georg Fischer AG',
        department: 'Local IT',
        role: 'IT Guy in apprentice',
        description:
          'I am working in the IT department and I am responsible for the support of the employees. I am also working on projects and I am learning a lot about the IT infrastructure of a big company.',
      },
      gfOpex: {
        company: 'Georg Fischer AG',
        department: 'OpEx & Production IT',
        role: 'IT Guy in apprentice',
        description:
          'Working on various projects focused on data analysis and data science initiatives. Actively developing expertise in Microsoft Power BI and Power Platform implementation for business intelligence solutions. Now and then I also doing support for the Infrastructure in the production',
      },
      gfDzs: {
        company: 'Georg Fischer AG',
        department: 'DZS IT',
        role: 'IT Guy in apprentice',
        description:
          'Fixing bugs, implementing new features into UniWare LVS and support the users of the Distribution Center.',
      },
      gfDzsFix: {
        company: 'Georg Fischer AG',
        department: 'DZS IT',
        role: 'Software Engineer & System Administrator',
        description:
          'Developing and fixing bugs in UniWare LVS and administrating the IT infrastructure in the production.',
      },
      bbz: {
        school: 'BBZ Schaffhausen',
        degree: 'Business IT specialist EFZ',
        description:
          'This is the education I am doing as part of my apprenticeship at Georg Fischer.',
      },
    },
  },
  footer: {
    links: {
      home: 'Home',
      projects: 'Projects',
      blog: 'Blog',
      contact: 'Contact',
      career: 'Career',
      recipes: 'Recipes',
      uses: 'What i use',
    },
    category: {
      other: 'Other',
      importantLinks: 'Important Links',
      socials: 'Socials',
    },
  },
  recipes: {
    filter: {
      searchPlaceholder: 'Search recipes by name, description, ingredients, or kitchen...',
      kitchen: 'Kitchen',
      search: 'Search',
      all: 'All',
      selectKitchen: 'Select Kitchen',
      allergens: 'Allergens',
      none: 'None',
      exclude: 'Exclude',
      clear: 'Clear',
      sort: 'Sort',
      sortBy: 'Sort By',
      order: 'Order',
      resetFilters: 'Reset Filters',
      sortOrder: 'Sort Order',
      sortByTime: 'Time',
      sortByCreated: 'Created',
      sortByKitchen: 'Kitchen',
      sortByName: 'Name',
      sortOrderAsc: 'Ascending',
      sortOrderDesc: 'Descending',
      noRecipesMatchFilters: 'No recipes match your filters.',
      results: 'results',
    },
    allergens: {
      lactose: 'Lactose',
      gluten: 'Gluten',
      egg: 'Egg',
      soy: 'Soy',
      nuts: 'Nuts',
    },
    title: 'Recipes',
    description: 'A collection of recipes I like.',
    units: {
      ml: 'ml',
      cl: 'cl',
      dl: 'dl',
      l: 'l',
      g: 'g',
      kg: 'kg',
      tsp: 'tsp',
      tbsp: 'tbsp',
      pcs: 'pcs',
      pinch: 'pinch',
      clove: 'clove',
      can: 'can',
      cup: 'cup',
      leaf: 'leaf',
      slice: 'slice',
    },
    unitTypes: {
      fluid: 'Fluid',
      weight: 'Weight',
      spoon: 'Spoon',
      arbitrary: 'Arbitrary',
      ingredient: 'Ingredient',
      plural: {
        fluid: 'Fluids',
        weight: 'Weights',
        spoon: 'Spoons',
        arbitrary: 'Arbitraries',
        ingredient: 'Ingredients',
      },
    },
    types: {
      bread: 'Bread',
      sauce: 'Sauce',
      salad: 'Salad',
      soup: 'Soup',
      dessert: 'Dessert',
      drink: 'Drink',
      snack: 'Snack',
      main: 'Main',
    },
    titles: {
      ingredients: 'Ingredients',
      instructions: 'Instructions',
      servings: 'Servings',
      preparationTime: 'Preparation Time',
      cookingTime: 'Cooking Time',
      totalTime: 'Total Time',
      kitchenTools: 'Kitchen Tools',
      related: 'Related',
      story: 'The Story',
    },
    shoppingList: {
      title: 'Shopping List',
      items: 'items',
      empty: 'No items in shopping list',
      viewList: 'View Shopping List',
      optional: 'Optional',
      for: 'for',
      select: 'Select for shopping list',
      deselect: 'Deselect',
      all: 'All Items',
      storage: {
        pantry: 'Pantry',
        refrigerator: 'Refrigerator',
        freezer: 'Freezer',
        spiceRack: 'Spice Rack',
        counter: 'Counter',
        other: 'Other',
      },
    },
    inventory: {
      title: 'My Inventory',
      description: 'Track what ingredients you have at home',
      empty: 'No items in inventory',
      addItem: 'Add Item',
      add: 'Add',
      selectIngredient: 'Select ingredient',
      quantity: 'Quantity',
      unit: 'Unit',
    },
    kitchenTools: {
      blender: 'Blender',
      riceCooker: 'Rice Cooker',
    },
    kitchen: {
      japanese: 'Japanese',
      indian: 'Indian',
      western: 'Western',
      asian: 'Asian',
      latinAmerican: 'Latin American',
      middleEastern: 'Middle Eastern',
      african: 'African',
      oceanic: 'Oceanic',
      caribbean: 'Caribbean',
      swiss: 'Swiss',
      selfmade: 'Selfmade',
    },
    ingredients: {
      chickenBreast: 'Chicken Breast',
      garlic: 'Garlic',
      corianderPowder: 'Coriander Powder',
      cuminPowder: 'Cumin Powder',
      curryPowder: 'Curry Powder',
      turmericPowder: 'Turmeric Powder',
      yogurt: 'Yogurt',
      butter: 'Butter',
      onion: 'Onion',
      tomato: 'Tomato',
      cream: 'Cream',
      springOnion: 'Spring Onion',
      zweifelPaprikaPowder: 'Zweifel Paprika Powder',
      egg: 'Egg',
      rice: 'Rice',
      water: 'Water',
      soySauce: 'Soy Sauce',
      salt: 'Salt',
      carrot: 'Carrot',
      potato: 'Potato',
      beefMince: 'Beef Mince',
      noriSeaweed: 'Nori Seaweed',
      zucchini: 'Zucchini',
    },
    recipes: {
      indianButterChicken: {
        name: 'Indian Butter Chicken',
        description:
          'Indian Butter Chicken is a delicious dish that is made with chicken and butter.',
        instructions: [
          'Cut the chicken breast into small pieces and marinate them with the spices and yogurt.',
          'Spread the chicken out on a baking sheet and bake it in the convection oven at 220°C until tender.',
          'Fry the Pressed Garlic with the roughly chopped onions until golden brown.',
          'Add the roughly chopped tomatoes, add the spices and cook until soft. Optionally, add some spiciness with chili flakes.',
          'Blend the sauce until smooth and put it back in the pan.',
          'Add the cream and some yogurt to taste.',
          'Add the chicken to the sauce and cook until heated through.',
          'Serve with rice or naan bread.',
        ],
      },
      riceCookerOnePot: {
        name: 'Rice Cooker One Pot',
        description: 'Simple and easy rice cooker recipe.',
        instructions: [
          'Wash the rice and drain the rice with cold water.',
          'Add the rice and water to the rice cooker.',
          'Add the salt and soy sauce to the rice cooker and stir.',
          'Add chopped chicken breast and spring onion to the rice cooker and spread it evenly.',
          'Start the rice cooker and wait 10 minutes.',
          'Whisk the eggs and pour them on top of the rest.',
          'Wait until the rice is cooked and serve.',
        ],
      },
      paprikaRice: {
        name: 'Paprika Rice',
        description: 'Easy selfmade rice recipe.',
        instructions: [
          'For the rice, follow the instructions in the related Rice Recipe.',
          'Marinade the chicken breast with the paprika powder',
          'Put the chicken breast in a pan with the butter and cook until the chicken is completely cooked.',
          'Add the cream to the pan and stir until the cream is heated through.',
          'Serve with the rice.',
        ],
      },
      coreaRice: {
        name: 'Corea Rice',
        description: 'A family recipe passed down through generations.',
        story:
          'This is a recipe my grandmother first taught my father and now I am sharing it with you. It originally comes from a corean woman who lived with my grandparents for a while many years back. And while she was there, she taught my grandmother how to make this recipe.',
        instructions: ['Follow the basic rice cooking instructions from the related Rice Recipe.'],
      },
    },
  },
}
