...
import { Calendar, ChevronDown, SquareArrowOutUpRight } from "lucide-react";
...
const WakaTimeStats = () => {
  ...
  const [stats, setStats] = useState<WakaTimeStatsDef | null>(null);
  const t = useTranslations("wakatime");
  const format = useFormatter();
  const [allTimeStats] = useState<boolean>(true);
  useEffect(() => {
    if (data) {
      setStats(data.allTimeStats);
    }
  }, [data]);
        ...
        <div className="mb-8">
          <div className="flex gap-4 items-center">
            ...
            {/* <div className="flex justify-between items-center">
              <div className="flex items-center gap-2">
                <Button
                  variant="outline"
                  onClick={() => setAllTimeStats(!allTimeStats)}
                >
                  <Calendar className="h-4 w-4" />
                  {allTimeStats ? t("allTime") : t("last7Days")}
                  <ChevronDown
                    className={cn(
                      "h-4 w-4 transition-transform duration-200",
                      allTimeStats ? "rotate-180" : ""
                    )}
                  />
                </Button>
              </div>
          ...
          <p className="mt-2 text-gray-400">
            {allTimeStats
              ...
              : t("totalCodingTimeWeek")}
          </p>
        </div>

        <div className="mb-8 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          <div className="rounded-lg bg-gradient-to-br from-blue-500/10 to-purple-500/10 p-6 backdrop-blur-sm">
          ...
          <div className="rounded-lg bg-gradient-to-br from-purple-500/10 to-pink-500/10 p-6 backdrop-blur-sm">
            <h3 className="text-sm font-medium text-gray-400">
              {t("totalLanguages")}
            </h3>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={stats.languages.slice(0, 10)}>
                    ...
                    <Tooltip
                      ...
                      formatter={(_, name, props) => [props.payload.text, name]}
                    />
                    <Bar
