...
const Posts = async ({ locale }: { locale: string }) => {
  ...
  return (
    <section className="overflow-hidden h-full flex flex-col items-start justify-start max-w-5xl">
      ...
      <Header2>{t("allPosts")}</Header2>

      {!posts.length && <p className="mb-4 text-red-400">{t("noPosts")}</p>}
      <div className="relative mt-3 ml-3 flex flex-col items-start justify-center border-l border-gray-600">
            ...
            <div>
              ...
              <div>
                <h3 className="mb-1 flex items-center text-lg font-bold text-gray-200">
                  {post.metadata.title}{" "}
                  {index === 0 && (
                    <span className="mr-2 ml-3 hidden rounded-sm bg-black/10 py-0.5 pr-2.5 pl-1.5 text-sm font-medium sm:block bg-white/10">
                      🔥 {t("latestPost")}
                    </span>
                  )}
                </h3>
              ...
              {Array.isArray(post.metadata.tech) &&
                post.metadata.tech.length > 0 && (
                  <div className="flex flex-wrap gap-1 mb-2">
                    {post.metadata.tech
                      .filter((tech) => TECH.includes(tech))
                      .map((tech) => (
